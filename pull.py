...
class YoudaoNoteSession(requests.Session):
    ...
    FILE_URL_DOCX = 'https://note.youdao.com/ydoc/api/personal/doc?method=download-docx&fileId=%s&cstk=%s&keyfrom=web'
    ...
    def ready_get_file(self, id, name, local_dir, file_entry, custom_suffix) -> None:
        ...
        # 如果有道云笔记是「笔记」类型，则设置提示类型,获取的笔记文件可能没有后缀
        print("处理文件「%s」后缀" % name)
        tip = '，云笔记原格式为 .note'
        local_file_path = os.path.join(local_dir, name)  # 用于将后缀 .note 转换为其他后缀
        local_file_basename = os.path.join(local_dir, os.path.splitext(name)[0])  # 获取没有后缀的文件名
        local_file_path = local_file_basename + custom_suffix

        # 下载文件,使用 try 进行异常处理
        try:
            self.get_file(id, local_file_path, custom_suffix)
            print('新增「%s」%s' % (local_file_path, tip))
        except Exception as error:
            print('error：get_file() 函数处理「%s」时出错' % local_file_path)
    ...
    def get_file(self, file_id, file_path, youdao_file_suffix) -> None:
        ...
        # .docx 的接口与普通接口不一样，需要的参数不同。
        if youdao_file_suffix == '.docx':
            url = self.FILE_URL_DOCX % (file_id, self.cstk)
            response = self.get(url)
        else:
...
def main():
    ...
    try:
        ...
        print('正在 pull，请稍后 ...')
        session.get_all(config_dict['local_dir'], config_dict['ydnote_dir'], config_dict['smms_secret_token'], root_id, config_dict['suffix'])
